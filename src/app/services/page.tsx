...
const page = () => {
	return (
		<div className='min-h-screen py-12'>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mx-[20px] mt-40 ">
				{servicesData.map((service) => (
					<div
						key={service.id}
						className="relative flex flex-col items-center justify-start text-left  shadow-lg glass-effect p-8 pt-20 bg-foreground/60 transition-all duration-300 hover:scale-105 hover:shadow-blue-500/20"

					>
						{/* Floating image above card */}
						<div className="absolute -top-16 left-1/2 -translate-x-1/2 w-32 h-32 flex items-center justify-center z-10">
							<Image src={service.image} width={120} height={120} alt={service.title} className="object-contain drop-shadow-xl" />
						</div>
						<h2 className="text-2xl font-bold mb-3 mt-2 text-sky" >{service.title}</h2>
						<p className="mb-8 text-base leading-relaxed text-foreground/70">{service.description}</p>
						<button
							className="flex items-center gap-2 font-medium py-2 px-6 rounded-lg   transition-colors duration-200 shadow text-foreground "

						>
							Read more
							<span
								className="w-5 h-5 rounded-md flex items-center justify-center bg-foreground/30 "
							>
